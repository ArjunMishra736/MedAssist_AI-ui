...
# ── Credentials ───────────────────────────────────────────────────────────────
VALID_USERNAME = "Arjunmedico"
VALID_PASSWORD = "Healthprediction"
...
<style>
    ...
    .login-icon  { font-size: 3.5rem; text-align: center; }
    .login-title {
        font-size: 2rem; font-weight: 700; color: #0f4c81;
    ...
    .error-box {
        ...
        color: #c53030; font-size: 0.88rem; margin-bottom: 1rem;
    }
    .login-footer {
    ...
    div[data-testid="stTextInput"] input:focus {
        ...
        box-shadow: 0 0 0 3px rgba(26,122,191,0.15) !important;
    }
    /* Login button — blue filled */
    div[data-testid="stButton"]:first-of-type > button {
        width: 100% !important;
        background: linear-gradient(135deg, #0f4c81, #1a7abf) !important;
...
def show_login():
    ...
    # Centre using columns
    _, col, _ = st.columns([1, 1.5, 1])
    with col:
        st.markdown('<div class="login-icon">🩺</div>',         unsafe_allow_html=True)
        st.markdown('<div class="login-title">MedAssist AI</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="login-subtitle">AI-powered Disease Prediction System</div>',
        ...
        if st.session_state.login_error:
            st.markdown(
                f'<div class="error-box">🔒 {st.session_state.login_error}</div>',
                unsafe_allow_html=True,
            )
        ...
        password = st.text_input("🔑  Password", placeholder="Enter your password",
                                 type="password", key="p_input")

        if st.button("Login to MedAssist AI", use_container_width=True):
        ...
        st.markdown(
            ...
            "🔐 Secure access — authorized personnel only<br>"
            "132 Symptoms &nbsp;•&nbsp; 41 Diseases &nbsp;•&nbsp; Random Forest AI"
            "</div>",
            unsafe_allow_html=True,
        )
...
def show_app():
    ...
    # ── Sidebar ───────────────────────────────────────────────────────────────
    with st.sidebar:
        st.markdown("### 👋 Welcome, Arjun!")
        st.caption("Logged in as **Arjunmedico**")
        st.markdown("---")
    ...
    with tab1:
        ...
        symptom_categories = {
            "🌡️ General":         ["fever","fatigue","chills","weakness","weight_loss",
                                    "weight_gain","night_sweats"],
            "🫁 Respiratory":     ["cough","shortness_of_breath","sore_throat","runny_nose",
                                    "nasal_congestion","post_nasal_drip","hoarseness","wheezing",
                                    "chest_tightness","cough_blood","difficulty_breathing",
                                    "rapid_breathing","slow_breathing","loss_of_smell"],
            "🤢 Digestive":       ["nausea","vomiting","diarrhea","abdominal_pain","constipation",
                                    "bloating","gas","heartburn","acid_reflux","loss_of_appetite",
                                    "difficulty_swallowing","blood_in_stool","pale_stool"],
            "❤️ Cardiovascular":  ["chest_pain","palpitations","high_bp","low_bp",
                                    "irregular_heartbeat","fainting","swelling"],
            "🧠 Neurological":    ["headache","dizziness","confusion","memory_loss","tremors",
                                    "numbness","tingling","seizures","speech_difficulty",
                                    "loss_of_balance","difficulty_walking","migraine",
            ...
            "🦴 Musculoskeletal": ["muscle_pain","joint_pain","back_pain","neck_pain",
                                    "shoulder_pain","hip_pain","knee_pain","ankle_pain",
                                    "finger_pain","hand_stiffness","bone_pain","leg_cramps"],
            "🩺 Metabolic":       ["frequent_urination","thirst","dry_mouth","high_blood_sugar",
                                    "low_blood_sugar","excess_hunger","heat_intolerance",
                                    "cold_intolerance"],
            "🎗️ Skin":            ["skin_rash","itching","dry_skin","oily_skin","yellow_skin",
                                    "excess_sweating","hair_loss","skin_ulcers",
                                    "slow_healing_wounds","lump"],
            "🔬 Other":           ["yellow_eyes","dark_urine","blood_in_urine","painful_urination",
                                    "urinary_retention","urinary_incontinence","cold_hands",
                                    "cold_feet","foot_swelling","frequent_infections",
                                    "gum_bleeding","bad_breath","mouth_ulcers","tooth_pain",
                                    "snoring","sleep_apnea","heavy_menstruation","missed_periods",
                                    "pelvic_pain","breast_pain","breast_discharge",
                                    "erectile_dysfunction","testicular_pain","scrotal_swelling",
                                    "developmental_delay","growth_delay","learning_difficulty",
                                    "loss_of_taste"],
        }
        ...
        if predict_btn:
            if len(selected_symptoms) < 2:
                ...
                if severity.startswith("🔴"):
                    ...
                    st.warning("⚠️ **Please consult a doctor soon.** "
                               "This condition needs professional evaluation.")

                st.markdown("""
                <div class="warning-card">
                    ⚠️ <strong>Medical Disclaimer:</strong> This prediction is generated by an
                    AI model trained on 5,000 patient records. It is for informational purposes
                    only and <strong>NOT a substitute for professional medical diagnosis.</strong>
    ...
    with tab2:
        ...
        df_ref = pd.DataFrame([
            {"Disease":  k.replace("_", " "),
             "Medicines": v,
             "Severity":  SEVERITY_MAP.get(k, "🟡 MODERATE")}
    ...
    with tab3:
        ...
        c2.metric("Trees",     "200")
        c3.metric("Diseases",  "41")
        c4.metric("Symptoms",  "132")
        st.markdown("""
        #### How It Works
